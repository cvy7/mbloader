...
uint8_t ASCII_HEX_to_dec(uint8_t decChar){
    //Discard left side of BCD
    if(decChar<58){
        //ASCII [0,1,2...9]
        decChar -= 48;
...
{
    uint8_t lrc = LRC(req, req_length);
    req[req_length++] = lrc;
...
static ssize_t _modbus_rtu_send(modbus_t *ctx, const uint8_t *req, int req_length)
{
    ...
    if (ctx_rtu->rts != MODBUS_RTU_RTS_NONE) {
        ...
        ctx_rtu->set_rts(ctx, ctx_rtu->rts != MODBUS_RTU_RTS_UP);
        if(ctx->echo){
            int n_read=read(ctx->s, req, req_length);
            if (ctx->debug) {
                fprintf(stderr, "Reading request of [%d] chars with timeout [%d]us...\n",n_read,timeout);
            }
        }
...
static ssize_t _modbus_ascii_send(modbus_t *ctx, const uint8_t *req, int req_length)
{
    ...
    for(i=0;i<req_length;i++){
        uint8_t byte=*req++;
        buf[j++]=dec_to_ASCII_HEX(byte>>4);
        buf[j++]=dec_to_ASCII_HEX(byte);
    }
    buf[j++]='\r';
    buf[j++]='\n';
    ...
    modbus_rtu_t *ctx_rtu = ctx->backend_data;
    DWORD n_bytes = 0;
    return (WriteFile(ctx_rtu->w_ser.fd, buf, j, &n_bytes, NULL)) ? (ssize_t)req_length : -1;
#else
#if HAVE_DECL_TIOCM_RTS
    modbus_rtu_t *ctx_rtu = ctx->backend_data;
    if (ctx_rtu->rts != MODBUS_RTU_RTS_NONE) {
...
{
    int rcvd_bytes,rcv_bytes_to_read,i=0,j=0;
    uint8_t buf[2];

    if(rsp_length==_MODBUS_ASCII_SYN_LENGTH){//синхронизация ':' -> ':'
        rcv_bytes_to_read=1;
    }

    else rcv_bytes_to_read=2;//rsp_length*2;//'A','F'-> 0xAF

   // if(rcv_bytes_to_read>512) rcv_bytes_to_read=512;

#if defined(_WIN32)
    rcvd_bytes= win32_ser_read(&((modbus_rtu_t *)ctx->backend_data)->w_ser, buf, rcv_bytes_to_read);
#else
    rcvd_bytes=0;
    for(i=0;i<64;i++){
    //rcvd_bytes+= read(ctx->s, &buf[rcvd_bytes], (rcv_bytes_to_read-rcvd_bytes));

    if(rcv_bytes_to_read==1) {
        rcvd_bytes= read(ctx->s, &buf[0], 1);
        if(rcvd_bytes && buf[0]==':') break;
    }
    else{
        rcvd_bytes+= read(ctx->s, &buf[rcvd_bytes], (rcv_bytes_to_read-rcvd_bytes));
        if(rcvd_bytes>=rcv_bytes_to_read) break; 
    }
    usleep(1000);
    }
#endif
    if (ctx->debug) {
            printf("#R%dR B%dB#",rcv_bytes_to_read,rcvd_bytes);
    ...
    if(rsp_length==_MODBUS_ASCII_SYN_LENGTH){
       *rsp=buf[0];
       if(buf[0]==':') return 1;//_MODBUS_ASCII_SYN_LENGTH;
       else return 0;
    }

    //for(i=0;i<rsp_length;i++){
        uint8_t byte=ASCII_HEX_to_dec(buf[0]);
        byte=byte<<4;
        byte|=ASCII_HEX_to_dec(buf[1]);
        *rsp++=byte;
    //}

    return 1;//(rcvd_bytes/2);
}
...
/* The check_lrc function shall return 0 is the message is ignored and the
   message length if the CRC is valid. Otherwise it shall return -1 and set
   errno to EMBBADCRC. */
                                       ...
                                       const int msg_length)
{
    ...
    crc_calculated = LRC(msg, msg_length - 2);
    crc_received =  msg[msg_length - 2];
                         ...
                         int stop_bit)
{
    ...
    _modbus_init_common(ctx);
    //modbus_set_byte_timeout(ctx, 1, 999999);
    //modbus_set_response_timeout(ctx, 1, 999999);
    ...
    ctx_ascii = (modbus_rtu_t *)ctx->backend_data;
...
#if HAVE_DECL_TIOCSRS485
    /* The RS232 mode has been set by default */
    ctx_ascii->serial_mode = MODBUS_RTU_RS232;
#endif

#if HAVE_DECL_TIOCM_RTS
    /* The RTS use has been set by default */
    ctx_ascii->rts = MODBUS_RTU_RTS_NONE;

    /* Calculate estimated time in micro second to send one byte */
    ctx_ascii->onebyte_time = 2000000 * (1 + data_bit + (parity == 'N' ? 0 : 1) + stop_bit) / baud;

    /* The internal function is used by default to set RTS */
    ctx_ascii->set_rts = _modbus_rtu_ioctl_rts;
